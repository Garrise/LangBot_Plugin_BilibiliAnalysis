...
class BiliAnalysisPlugin(BasePlugin):
    ...
    async def group_normal_message_received(self, ctx: EventContext):
        ...
        if bv_match or av_match:
            ...
            #处理非永久短链
            url_match = re.search(r'(https://b23.tv/\w+)', msg) or re.search(r'(https://bili2233.cn/\w+)', msg)
            if url_match:
                url_parse = url_match.group(1)
            response = requests.head(url_parse, allow_redirects=True)
            if 'location' in response.headers:
                url = response.headers['location']
            else:
                url = response.url
            bv_match = re.search(r'www.bilibili.com/video/(BV\w+)', url)
            if bv_match:
                id = bv_match.group(1)
                req = f"bvid={id}"
                headers = {
